...
class VariantCardSelects extends HTMLElement {
  constructor() {
    super();
  }
  ...
  updateSelectionMetadata({ target }) {
    ...
    if (tagName === 'SELECT' && target.selectedOptions.length) {
      ...
      if (variantId) {
        ...
        if (sectionId) {
          const variantImages = document.querySelectorAll(`img[data-section="${sectionId}"][data-variant-id="${variantId}"]`);
          
          if (variantImages.length > 0) {
            const allSectionImages = document.querySelectorAll(`img[data-section="${sectionId}"]`);
...
customElements.define('variant-card-selects', VariantCardSelects);
